...
unsigned char* DES::encrypt(const unsigned char* plaintext)
{
	...
	unsigned char* crypt = new unsigned char[8];
	ltoc(block[0], crypt);
	ltoc(block[1], crypt + 4);
	cout << "Crypt part is: '" << crypt << "'\n";
	return crypt;
}
...
unsigned char* DES::decrypt(const unsigned char* ciphertext)
{
	...
	unsigned char crypt[8];
	for(int i = 0; i < 8; i++)
	{
		crypt[i] = *(ciphertext + i);
	}
	unsigned char* tmp = crypt;
	...
	unsigned char* plain = new unsigned char[8];
	ltoc(block[0], plain);
	ltoc(block[1], plain + 4);
	cout << "plain part is: '" << plain << "'\n";
	return plain;
}
